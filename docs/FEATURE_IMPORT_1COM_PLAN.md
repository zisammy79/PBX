...
| 4 VM / MoH / conf / phonebooks | **PARTIAL** | Conferences/paging/phonebooks CRUD+UI; MoH CRUD+UI with media multi-select + PATCH `mediaFileIds`; telephony-config emits `musiconhold-tenants.conf` + queue `musiconhold=` binding; **MoH media sync** on activate copies into `infrastructure/asterisk/generated/moh/{class}/` (Asterisk path `/etc/asterisk/pbx-generated/moh/...`) |
...
| 6 campaigns / DNC / cron | **PARTIAL** | DNC lists API; campaigns CRUD+UI; `POST/GET .../campaigns/:id/numbers` (import with optional `skipDnc`, paginated list) + paste UI; voice dialer tick in `@pbx/database` + **worker cron**; `CampaignOriginateService` stub defers with `campaign.dial.deferred_no_originate` unless `CAMPAIGN_ORIGINATE_ENABLED=true` |
| 7 BLF / fax / SMS | **PARTIAL** | Button layouts CRUD+assign+UI; **provisioning file download** (`GET .../provisioning-file?vendor=`) generic JSON / Yealink XML / Fanvil stub; fax CRUD+UI; SMS messages stub; SMS campaign start warns when no provider |
| 8 docs / QA / isolation | **PARTIAL** | Phase 7 isolation **PASS**; **five-tenant callflow matrix** (`five-tenant-callflow.isolation.integration.spec.ts`) covers media/queues/campaigns/faxes/button-layouts/voicemails |
