...
import { extensionStatusEnum, sipTransportEnum } from './enums.js';
...
export const phoneNumbers = pgTable(
  'phone_numbers',
  {
    ...
    providerSid: varchar('provider_sid', { length: 34 }),
    status: varchar('status', { length: 64 }).notNull().default('active'),
    capabilities: jsonb('capabilities').notNull().default({}),
    regulatoryStatus: varchar('regulatory_status', { length: 128 }),
    outboundCallerIdPolicy: jsonb('outbound_caller_id_policy').notNull().default({}),
...
export const queues = pgTable(
  'queues',
  {
    ...
    recordAlways: boolean('record_always').notNull().default(false),
    mohClassId: uuid('moh_class_id'),
    createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
    updatedAt: timestamp('updated_at', { withTimezone: true }).notNull().defaultNow(),
  },
  (table) => [
...
export const ivrs = pgTable(
  'ivrs',
  {
    ...
    allowDialExtensions: boolean('allow_dial_extensions').notNull().default(false),
    allowFeatureCodes: boolean('allow_feature_codes').notNull().default(false),
...
export const businessSchedules = pgTable(
  'business_schedules',
  {
    ...
    scheduleType: varchar('schedule_type', { length: 32 }).notNull().default('weektime'),
    rules: jsonb('rules').notNull().default([]),
    openDestinationType: varchar('open_destination_type', { length: 64 }),
    openDestinationId: uuid('open_destination_id'),
    closedDestinationType: varchar('closed_destination_type', { length: 64 }),
    closedDestinationId: uuid('closed_destination_id'),
    createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
    updatedAt: timestamp('updated_at', { withTimezone: true }).notNull().defaultNow(),
  },
